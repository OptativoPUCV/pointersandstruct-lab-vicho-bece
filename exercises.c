...
Persona* crearPersona(char nombre[], char rut[], int edad) {
  Persona * a = (Persona *)malloc(sizeof(Persona) * 1);
...
Vector * crearVector(int n) {
  Vector * largo = (Vector *)malloc(sizeof(Vector) * n);
  int i;
  for(i = 0; i < n; i++){
    largo[i].datos = (int *) calloc (n,sizeof(int));
    largo[i].capacidad = n;
  }
   return largo;
}
